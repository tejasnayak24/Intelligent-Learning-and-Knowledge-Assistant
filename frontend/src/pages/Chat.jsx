...
export default function Chat() {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      // LIVE CONNECTED: Await response from backend vector engine
      const aiResponse = await chatService.askQuestion(userQuery);
      
      setMessages((prev) => [...prev, { text: aiResponse, isBot: true, sender: 'ai', sources: [] }]);
    } catch (err) {
        ...
        <header className="h-16 border-b border-slate-800/80 bg-slate-900/40 px-6 flex items-center bg-blend-normal backdrop-blur-md">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">RAG Analysis Engine Online</p>
            </div>
          </div>
        </header>
